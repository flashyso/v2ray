...
if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/flashy_top/v2ray ]]; then

	. $backup

elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/flashy_v2ray_backup.txt && -d /etc/v2ray/flashy_top/v2ray ]]; then

	. /etc/v2ray/flashy_top/v2ray/tools/v1xx_to_v3xx.sh
...
if [[ $mark != "v3" ]]; then
	. /etc/v2ray/flashy_top/v2ray/tools/v3.sh
fi
...
. /etc/v2ray/flashy_top/v2ray/src/init.sh
systemd=true
# _test=true
...
create_vmess_URL_config() {
	...
	if [[ $v2ray_transport == [45] ]]; then
		...
		cat >/etc/v2ray/vmess_qr.json <<-EOF
			vless://${v2ray_id}@${domain}:443?encryption=none&security=tls&type=ws&host=${domain}&path=${_path}#flashy_${domain}
		EOF
	else
		[[ -z $ip ]] && get_ip
		cat >/etc/v2ray/vmess_qr.json <<-EOF
...
get_v2ray_config_info_link() {
	...
	local link=$(curl -s --upload-file /tmp/flashy_v2ray.txt "https://transfer.sh/${random}_flashy_v2ray.txt")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 配置信息链接-------------"
		echo
...
update_v2ray.sh() {
	...
	if [[ $latest_version == $_version ]]; then
		...
		echo
		cd /etc/v2ray/flashy_top/v2ray
		git pull
		cp -f /etc/v2ray/flashy_top/v2ray/v2ray.sh $_v2ray_sh
		chmod +x $_v2ray_sh
		echo
