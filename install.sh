...
_load() {
	local _dir="/etc/v2ray/flashyso/v2ray/src/"
	. "${_dir}$@"
}
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		mkdir -p /etc/v2ray/flashyso/v2ray
		cp -rf $(pwd) /etc/v2ray/flashyso/v2ray/
	else
		pushd /tmp
		git clone --depth=1 https://github.com/flashyso/v2ray -b "$_gitbranch" /etc/v2ray/flashyso/v2ray
		popd

	fi

	if [[ ! -d /etc/v2ray/flashyso/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/flashyso/v2ray/config/backup.conf $backup
