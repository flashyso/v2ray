...
_load() {
	local _dir="/etc/v2ray/flashy_top/v2ray/src/"
	. "${_dir}$@"
}
...
path_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}flashy$none]):")" path
		[[ -z $path ]] && path="flashy"
...
proxy_site_config() {
	echo
	while :; do
		echo -e "请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://wz.flashy.top"
		echo -e "举例...你当前的域名是 $green$domain$none , 伪装的网址的是 https://wz.flashy.top"
		echo -e "然后打开你的域名时候...显示出来的内容就是来自 https://wz.flashy.top 的内容"
		echo -e "其实就是一个反代...明白就好..."
		echo -e "如果不能伪装成功...可以使用 v2ray config 修改伪装的网址"
		read -p "$(echo -e "(默认: [${cyan}https://wz.flashy.top$none]):")" proxy_site
		[[ -z $proxy_site ]] && proxy_site="https://wz.flashy.top"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}flashy.top$none)"): " sspass
		[ -z "$sspass" ] && sspass="flashy.top"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		mkdir -p /etc/v2ray/flashy_top/v2ray
		cp -rf $(pwd) /etc/v2ray/flashy_top/v2ray/
	else
		pushd /tmp
		git clone --depth=1 https://github.com/233boy/v2ray -b "$_gitbranch" /etc/v2ray/flashy_top/v2ray
		popd

	fi

	if [[ ! -d /etc/v2ray/flashy_top/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/flashy_top/v2ray/config/backup.conf $backup
