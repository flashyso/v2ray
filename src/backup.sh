...
_first_backup(){
        ...
        dynamicPort)
            ...
            ;;
        domain)
            sed -i "s/domain=flashy.top/domain=$domain/" $backup
            ;;
        caddy)
            sed -i "s/caddy=/caddy=true/" $backup
            ;;
        +ss)
            sed -i "s/shadowsocks=/shadowsocks=true/; s/ssport=6666/ssport=$ssport/; s/sspass=flashy.top/sspass=$sspass/; s/ssciphers=chacha20-ietf/ssciphers=$ssciphers/" $backup
            ;;
        +ad)
            sed -i "s/ban_ad=/ban_ad=true/" $backup
            ;;
        +path)
            sed -i "s/path_status=/path_status=true/; s/path=flashy/path=$path/; s#proxy_site=https://wz.flashy.top#proxy_site=$proxy_site#" $backup
            ;;
        esac
    done
}